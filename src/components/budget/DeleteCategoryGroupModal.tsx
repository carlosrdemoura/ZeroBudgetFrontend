...
interface Props {
  ...
  onConfirm: (groupId: string, targetCategoryId: string) => Promise<void>;
  onClose: () => void;
}

export function DeleteCategoryGroupModal({ group, groups, onConfirm, onClose }: Props) {
    ...
    () =>
      groups.flatMap((g) =>
        g.id === group.id
          ? []
          : g.categories
              .filter((c) => c.id !== INFLOW_CATEGORY_ID)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await onConfirm(group.id, targetCategoryId);
      onClose();
    } catch (err: unknown) {
      const msg =
  ...
  return (
    ...
    >
      <div className="modal-box w-full sm:max-w-md">
        ...
        <h3 className="font-bold text-lg mb-1">Delete &ldquo;{group.name}&rdquo;?</h3>
        <p className="text-sm text-base-content/60 mb-4">
          All categories in this group will be removed and their transactions will be moved to the category you select below.
        </p>
            ...
            {noDestinations && (
              <div className="label pt-2">
                <span className="label-text-alt text-warning">
                  You need at least one category in another group to receive transactions before deleting this group.
                </span>
              </div>
            )}
          </label>
